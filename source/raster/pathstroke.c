...
static fz_error* line(struct sctx* s, float x0, float y0, float x1, float y1)
{
...
static fz_error* arc(struct sctx* s, float xc, float yc, float x0, float y0, float x1, float y1)
{
    fz_error* error;
    float     th0, th1, r;
    float     theta;
...
static fz_error* linestroke(struct sctx* s, fz_point a, fz_point b)
{
    fz_error* error;
...
static fz_error* linejoin(struct sctx* s, fz_point a, fz_point b, fz_point c)
{
    fz_error* error;
...
static fz_error* linecap(struct sctx* s, fz_point a, fz_point b)
{
    fz_error* error;
    ...
    if (linecap == SQUARE) {
        error = line(s, b.x - dlx, b.y - dly, b.x - dlx - dly, b.y - dly + dlx);
        if (error)
            return error;
        error = line(s, b.x - dlx - dly, b.y - dly + dlx, b.x + dlx - dly, b.y + dly + dlx);
        if (error)
            return error;
        error = line(s, b.x + dlx - dly, b.y + dly + dlx, b.x + dlx, b.y + dly);
        if (error)
            return error;
    }
...
static fz_error* linedot(struct sctx* s, fz_point a)
{
    fz_error* error;
    float     flatness  = s->flatness;
    float     linewidth = s->linewidth;
...
static fz_error* strokeflush(struct sctx* s)
{
    fz_error* error;
...
static fz_error* strokemoveto(struct sctx* s, fz_point cur)
{
    fz_error* error;
...
static fz_error* strokelineto(struct sctx* s, fz_point cur)
{
    fz_error* error;
...
static fz_error* strokeclosepath(struct sctx* s)
{
    fz_error* error;
...
static fz_error* strokebezier(struct sctx* s, float xa, float ya, float xb, float yb, float xc, float yc, float xd, float yd)
{
    fz_error* error;
    float     dmax;
    float     xab, yab;
...
static fz_error* dashmoveto(struct sctx* s, fz_point a)
{
    s->toggle = 1;
    s->offset = 0;
...
static fz_error* dashlineto(struct sctx* s, fz_point b)
{
    fz_error* error;
    float     dx, dy;
...
static fz_error* dashbezier(struct sctx* s, float xa, float ya, float xb, float yb, float xc, float yc, float xd, float yd)
{
    fz_error* error;
    float     dmax;
    float     xab, yab;
