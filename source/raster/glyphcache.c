...
void fz_dropglyphcache(fz_glyphcache* arena)
{
    fz_free(arena->hash);
    fz_free(arena->lru);
    fz_free(arena->buffer);
...
static fz_val* hashfind(fz_glyphcache* arena, fz_key* key)
{
...
static void hashinsert(fz_glyphcache* arena, fz_key* key, fz_val* val)
{
    fz_hash* tab     = arena->hash;
    int      pos     = hashkey(key) % arena->slots;
    int      didcoll = 0;
...
static void hashremove(fz_glyphcache* arena, fz_key* key)
{
    fz_hash*     tab = arena->hash;
    ...
    while (1) {
        ...
        if (memcmp(key, &tab[pos].key, sizeof(fz_key)) == 0) {
            ...
            while (tab[look].val) {
                ...
                if ((code <= hole && hole < look) || (look < code && code <= hole) || (hole < look && look < code)) {
                    tab[hole]          = tab[look];
...
void fz_debugglyphcache(fz_glyphcache* arena)
{
...
static void bubble(fz_glyphcache* arena, int i)
{
    fz_val tmp;
...
static void evictlast(fz_glyphcache* arena)
{
...
static void evictall(fz_glyphcache* arena)
{
    printf("zap!\n");
...
fz_error* fz_renderglyph(fz_glyphcache* arena, fz_glyph* glyph, fz_font* font, int cid, fz_matrix ctm)
{
    fz_error* error;
    fz_key    key;
    fz_val*   val;
