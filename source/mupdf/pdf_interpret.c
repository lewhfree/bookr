...
static void clearstack(pdf_csi* csi)
{
    int i;
    for (i = 0; i < csi->top; i++)
...
static fz_error* gsave(pdf_csi* csi)
{
    if (csi->gtop == 31)
...
static fz_error* grestore(pdf_csi* csi)
{
    if (csi->gtop == 0)
...
void pdf_dropcsi(pdf_csi* csi)
{
    ...
    if (csi->path)
        fz_dropnode((fz_node*)csi->path);
    if (csi->clip)
        fz_dropnode((fz_node*)csi->clip);
    if (csi->textclip)
        fz_dropnode((fz_node*)csi->textclip);
    if (csi->text)
        fz_dropnode((fz_node*)csi->text);
    if (csi->array)
        fz_dropobj(csi->array);
...
static fz_error* runxobject(pdf_csi* csi, pdf_xref* xref, pdf_xobject* xobj)
{
    fz_error*  error;
    fz_node*   transform;
    fz_stream* file;
...
static fz_error* runinlineimage(pdf_csi* csi, pdf_xref* xref, fz_obj* rdb, fz_stream* file, fz_obj* dict)
{
    fz_error*  error;
    pdf_image* img;
    char       buf[256];
...
static fz_error* runextgstate(pdf_gstate* gstate, pdf_xref* xref, fz_obj* extgstate)
{
    int i, k;
...
static fz_error* runkeyword(pdf_csi* csi, pdf_xref* xref, fz_obj* rdb, char* buf)
{
...
fz_error* pdf_runcsi(pdf_csi* csi, pdf_xref* xref, fz_obj* rdb, fz_stream* file)
{
    fz_error* error;
    char      buf[65536];
    int       token, len;
