...
static fz_error* parseobj(fz_stream* file, char* buf, int cap, int* stmofs, int* stmlen, int* isroot, int* isinfo)
{
    fz_error* error;
    fz_obj*   dict = nil;
    fz_obj*   length;
    ...
    *isinfo = 0;
    ...
    while (tok != PDF_TSTREAM && tok != PDF_TENDOBJ && tok != PDF_TERROR && tok != PDF_TEOF)
        tok = pdf_lex(file, buf, cap, &len);
...
fz_error* pdf_repairxref(pdf_xref* xref, char* filename)
{
    fz_error*  error;
    fz_stream* file;
    ...
    error = fz_packobj(&xref->trailer, "<< /Size %i /Root %r >>", maxoid + 1, rootoid, rootgen);
    if (error)
        goto cleanup;
