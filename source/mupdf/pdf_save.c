...
static fz_error* writestream(fz_stream* out, pdf_xref* xref, pdf_crypt* encrypt, int oid, int gen)
{
    fz_error*     error;
    fz_stream*    dststm;
    fz_stream*    srcstm;
...
static fz_error* writeobject(fz_stream* out, pdf_xref* xref, pdf_crypt* encrypt, int oid, int gen)
{
...
fz_error* pdf_updatexref(pdf_xref* xref, char* path)
{
    fz_error*  error;
    fz_stream* out;
    int        oid;
    ...
    while (oid < xref->len) {
        ...
        for (i = 0; i < n; i++) {
            ...
            fz_print(out, "%010d %05d %c \n", xref->table[oid + i].ofs, xref->table[oid + i].gen, xref->table[oid + i].type);
        }

        oid += n;
        while (oid < xref->len && xref->table[oid].type != 'a' && xref->table[oid].type != 'd')
            oid++;
    }
...
fz_error* pdf_savexref(pdf_xref* xref, char* path, pdf_crypt* encrypt)
{
    fz_error*  error;
    fz_stream* out;
    int        oid;
