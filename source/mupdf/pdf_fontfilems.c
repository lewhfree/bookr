...
static char* basepatterns[13] = {"CourierNewPSMT",    "CourierNewPS-BoldMT",    "CourierNewPS-ItalicMT",    "CourierNewPS-BoldItalicMT",    "ArialMT", "Arial-BoldMT", "Arial-ItalicMT", "Arial-BoldItalicMT",
                                 "TimesNewRomanPSMT", "TimesNewRomanPS-BoldMT", "TimesNewRomanPS-ItalicMT", "TimesNewRomanPS-BoldItalicMT", "SymbolMT"};
...
static void* localbsearch(const void* key, const void* base, size_t num, size_t width, int (*compare)(const void*, const void*))
{
    char*        lo = (char*)base;
...
static void removeredundancy(pdf_fontlistMS* fl)
{
    int i;
    int roffset          = 0;
    ...
    for (i = 0; i < fl->len; ++i)
        fprintf(stdout, "%s , %s , %d\n", fl->fontmap[i].fontface, fl->fontmap[i].fontpath, fl->fontmap[i].index);
}

static fz_error* swapword(char* pbyte, int nLen)
{
    int  i;
    char tmp;
    int  nMax;
...
static fz_error* decodeunicodeBMP(char* source, int sourcelen, char* dest, int destlen)
{
    wchar_t tmp[1024 * 2];
    int     converted;
...
static fz_error* decodeunicodeplatform(char* source, int sourcelen, char* dest, int destlen, int enctype)
{
    fz_error* err = nil;
    switch (enctype) {
    case UNI_ENC_UNI_1:
...
static fz_error* decodemacintoshplatform(char* source, int sourcelen, char* dest, int destlen, int enctype)
{
    fz_error* err = nil;
    switch (enctype) {
    case MAC_ROMAN:
...
static fz_error* decodemicrosoftplatform(char* source, int sourcelen, char* dest, int destlen, int enctype)
{
    fz_error* err = nil;
    switch (enctype) {
    case MS_ENC_SYMBOL:
...
static fz_error* growfontlist(pdf_fontlistMS* fl)
{
    int            newcap;
    ...
    memset(newitems + fl->cap, 0, sizeof(struct fz_keyval_s) * (newcap - fl->cap));
...
static fz_error* insertmapping(pdf_fontlistMS* fl, char* facename, char* path, int index)
{
    fz_error* err;
...
static fz_error* parseTTF(fz_file* file, int offset, int index, char* path)
{
    fz_error* err = nil;
    int       byteread;
        ...
        for (i = 0; i < ttNTHeader.uNRCount && err == nil; ++i) {
            ...
            if (ttRecord.uNameID == 6) {
                ...
                case PLATFORM_UNICODE:
                    err = decodeunicodeplatform(szTemp, ttRecord.uStringLength, szTemp, sizeof(szTemp), ttRecord.uEncodingID);
                    break;
                case PLATFORM_MACINTOSH:
                    err = decodemacintoshplatform(szTemp, ttRecord.uStringLength, szTemp, sizeof(szTemp), ttRecord.uEncodingID);
                    break;
                case PLATFORM_ISO:
                    err = fz_throw("fonterror : unsupported platform");
                    break;
                case PLATFORM_MICROSOFT:
                    err = decodemicrosoftplatform(szTemp, ttRecord.uStringLength, szTemp, sizeof(szTemp), ttRecord.uEncodingID);
                    break;
                }
...
static fz_error* parseTTFs(char* path)
{
    fz_error* err  = nil;
    fz_file*  file = nil;
...
static fz_error* parseTTCs(char* path)
{
    fz_error*       err = nil;
    int             byteread;
    fz_file*        file = nil;
    ...
    if (memcmp(fontcollectioin.Tag, "ttcf", sizeof(fontcollectioin.Tag)) == 0) {
        ...
        if (fontcollectioin.Version == TTC_VERSION1 || fontcollectioin.Version == TTC_VERSION2) {
...
fz_error* pdf_lookupfontMS(char* fontname, char** fontpath, int* index)
{
    pdf_fontmapMS  fontmap;
