...
fz_error* pdf_buildfillpath(pdf_gstate* gs, fz_pathnode* path, int eofill) { return fz_endpath(path, eofill ? FZ_EOFILL : FZ_FILL, nil, nil); }

static fz_error* addcolorshape(pdf_gstate* gs, fz_node* shape, fz_colorspace* cs, float* v)
{
    fz_error* error;
    fz_node*  mask;
    fz_node*  solid;
...
static fz_error* addinvisibleshape(pdf_gstate* gs, fz_node* shape)
{
    fz_error*    error;
    fz_node*     mask;
    fz_pathnode* path;
...
static fz_error* addpatternshape(pdf_gstate* gs, fz_node* shape, pdf_pattern* pat, fz_colorspace* cs, float* v)
{
    fz_error* error;
    fz_node*  xform;
    fz_node*  over;
    ...
    error = fz_packobj(&dict,
                       ...
                       " /Matrix[%f %f %f %f %f %f] >>",
                       pat->tree, pat->xstep, pat->ystep, pat->matrix.a, pat->matrix.b, pat->matrix.c, pat->matrix.d, pat->matrix.e, pat->matrix.f);
    if (error)
        return error;
...
static fz_error* addshadeshape(pdf_gstate* gs, fz_node* shape, fz_shade* shade)
{
    fz_error* error;
    fz_node*  mask;
    fz_node*  color;
...
fz_error* pdf_showimage(pdf_csi* csi, pdf_image* img)
{
    fz_error* error;
    fz_node*  mask;
    fz_node*  color;
...
fz_error* pdf_showpath(pdf_csi* csi, int doclose, int dofill, int dostroke, int evenodd)
{
...
fz_error* pdf_flushtext(pdf_csi* csi)
{
    pdf_gstate* gstate = csi->gstate + csi->gtop;
    fz_error*   error;
...
fz_error* showglyph(pdf_csi* csi, int cid)
{
    pdf_gstate* gstate = csi->gstate + csi->gtop;
    ...
    if (!csi->text || ((fz_font*)font) != csi->text->font || fabs(trm.a - csi->text->trm.a) > FLT_EPSILON || fabs(trm.b - csi->text->trm.b) > FLT_EPSILON || fabs(trm.c - csi->text->trm.c) > FLT_EPSILON ||
        fabs(trm.d - csi->text->trm.d) > FLT_EPSILON || gstate->render != csi->textmode) {
        error = pdf_flushtext(csi);
        if (error)
            return error;
...
void showspace(pdf_csi* csi, float tadj)
{
    pdf_gstate* gstate = csi->gstate + csi->gtop;
    pdf_font*   font   = gstate->font;
    if (font->super.wmode == 0)
...
fz_error* pdf_showtext(pdf_csi* csi, fz_obj* text)
{
    pdf_gstate*    gstate = csi->gstate + csi->gtop;
    pdf_font*      font   = gstate->font;
    fz_error*      error;
