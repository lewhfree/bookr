...
static fz_error* parseobj(fz_obj** obj, char** sp, struct vap* v)
{
    ...
    if (v != nil && *s == '%') {
        ...
        switch (*s) {
        case 'p':
            error = fz_newpointer(obj, va_arg(v->ap, void*));
            break;
        case 'o':
            *obj = fz_keepobj(va_arg(v->ap, fz_obj*));
            break;
        case 'b':
            error = fz_newbool(obj, va_arg(v->ap, int));
            break;
        case 'i':
            error = fz_newint(obj, va_arg(v->ap, int));
            break;
        case 'f':
            error = fz_newreal(obj, (float)va_arg(v->ap, double));
            break;
        case 'n':
            error = fz_newname(obj, va_arg(v->ap, char*));
            break;
        case 'r':
            oid   = va_arg(v->ap, int);
            gid   = va_arg(v->ap, int);
...
fz_error* fz_packobj(fz_obj** op, char* fmt, ...)
{
    fz_error*  error;
    struct vap v;
    va_list    ap;
...
fz_error* fz_parseobj(fz_obj** op, char* str) { return parseobj(op, &str, nil); }
