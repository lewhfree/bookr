...
fz_error* fz_newfaxe(fz_filter** fp, fz_obj* params)
{
    fz_obj* obj;
    ...
    if (obj)
        fax->endofline = fz_tobool(obj);

    obj = fz_dictgets(params, "EncodedByteAlign");
    if (obj)
        fax->encodedbytealign = fz_tobool(obj);

    obj = fz_dictgets(params, "Columns");
    if (obj)
        fax->columns = fz_toint(obj);

    obj = fz_dictgets(params, "EndOfBlock");
    if (obj)
        fax->endofblock = fz_tobool(obj);

    obj = fz_dictgets(params, "BlackIs1");
    if (obj)
        fax->blackis1 = fz_tobool(obj);
...
static void putbits(fz_faxe* fax, fz_buffer* out, int code, int nbits)
{
    while (nbits > 0) {
        if (fax->bidx == 0)
            *out->wp = 0;
...
static inline void putcode(fz_faxe* fax, fz_buffer* out, const cfe_code* run) { putbits(fax, out, run->code, run->nbits); }

static void putrun(fz_faxe* fax, fz_buffer* out, int run, int c)
{
    int m;
...
static fz_error* enc1d(fz_faxe* fax, unsigned char* line, fz_buffer* out)
{
    int run;
...
static fz_error* enc2d(fz_faxe* fax, unsigned char* ref, unsigned char* src, fz_buffer* out)
{
    int a1, a2, b1, b2;
    int run1, run2, n;
...
static fz_error* process(fz_faxe* fax, fz_buffer* in, fz_buffer* out)
{
    fz_error* error;
    int       i, n;
...
fz_error* fz_processfaxe(fz_filter* p, fz_buffer* in, fz_buffer* out)
{
    fz_faxe*  fax = (fz_faxe*)p;
    fz_error* error;
