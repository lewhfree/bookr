...
static fz_stream* newstm(int kind, int mode)
{
    fz_stream* stm;
...
static fz_error* openfile(fz_stream** stmp, char* path, int mode, int realmode)
{
    fz_error*  error;
    fz_stream* stm;
...
static fz_error* openfilter(fz_stream** stmp, fz_filter* flt, fz_stream* src, int mode)
{
    fz_error*  error;
    fz_stream* stm;
...
static fz_error* openbuffer(fz_stream** stmp, fz_buffer* buf, int mode)
{
    fz_stream* stm;
...
fz_error* fz_openrfile(fz_stream** stmp, char* path) { return openfile(stmp, path, FZ_SREAD, O_BINARY | O_RDONLY); }

fz_error* fz_openwfile(fz_stream** stmp, char* path) { return openfile(stmp, path, FZ_SWRITE, O_BINARY | O_WRONLY | O_CREAT | O_TRUNC); }
...
fz_error* fz_openrfilter(fz_stream** stmp, fz_filter* flt, fz_stream* src) { return openfilter(stmp, flt, src, FZ_SREAD); }

fz_error* fz_openwfilter(fz_stream** stmp, fz_filter* flt, fz_stream* src) { return openfilter(stmp, flt, src, FZ_SWRITE); }

fz_error* fz_openrbuffer(fz_stream** stmp, fz_buffer* buf) { return openbuffer(stmp, buf, FZ_SREAD); }

fz_error* fz_openwbuffer(fz_stream** stmp, fz_buffer* buf) { return openbuffer(stmp, buf, FZ_SWRITE); }
