...
fz_error* fz_newpredict(fz_filter** fp, fz_obj* params, int encode)
{
    fz_obj* obj;
...
void fz_droppredict(fz_filter* filter)
{
    fz_predict* p = (fz_predict*)filter;
    fz_free(p->ref);
}

static inline int getcomponent(unsigned char* buf, int x, int bpc)
{
    switch (bpc) {
    case 1:
        return buf[x / 8] >> (7 - (x % 8)) & 0x01;
    case 2:
        return buf[x / 4] >> ((3 - (x % 4)) * 2) & 0x03;
    case 4:
        return buf[x / 2] >> ((1 - (x % 2)) * 4) & 0x0f;
    case 8:
        return buf[x];
    }
    return 0;
...
static inline void putcomponent(unsigned char* buf, int x, int bpc, int value)
{
    ...
    case 1:
        ...
        break;
    case 2:
        buf[x / 4] |= value << ((3 - (x % 4)) * 2);
        break;
    case 4:
        buf[x / 2] |= value << ((1 - (x % 2)) * 4);
        break;
    case 8:
        buf[x] = value;
        break;
...
static inline void none(fz_predict* p, unsigned char* in, unsigned char* out) { memcpy(out, in, p->stride); }

static void tiff(fz_predict* p, unsigned char* in, unsigned char* out)
{
    int left[MAXC];
    int i, k;
...
static void png(fz_predict* p, unsigned char* in, unsigned char* out, int predictor)
{
    ...
    if (p->encode) {
        for (k = 0, i = 0; i < p->stride; k = (k + 1) % p->bpp, i++) {
            ...
            case 2:
                ...
                break;
            case 3:
                out[i] = in[i] - (left[k] + p->ref[i]) / 2;
                break;
            case 4:
                out[i] = in[i] - paeth(left[k], p->ref[i], upleft[k]);
                break;
            }
            left[k]   = in[i];
            upleft[k] = p->ref[i];
...
fz_error* fz_processpredict(fz_filter* filter, fz_buffer* in, fz_buffer* out)
{
...
fz_error* fz_newpredictd(fz_filter** fp, fz_obj* params) { return fz_newpredict(fp, params, 0); }

fz_error* fz_newpredicte(fz_filter** fp, fz_obj* params) { return fz_newpredict(fp, params, 1); }
