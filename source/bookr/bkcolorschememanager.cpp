...
BKColorSchemeManager::~BKColorSchemeManager() { BKUser::save(); }
...
int BKColorSchemeManager::update(unsigned int buttons)
{
    ...
    if (b[BKUser::controls.select] == 1) {
        if ((unsigned int)selItem == BKUser::options.colorSchemes.size() && selItem < 100) {
    ...
    if (b[BKUser::controls.alternate] == 1) {
        ...
        if (numSchemes > 1 &&       // Must have at least one scheme
            ...
            if (BKUser::options.currentScheme >= selItem && selItem > 0) { // but we do nothing in case we just deleted the first scheme
...
int BKColorSchemeManager::getColorScheme() { return colorScheme; }
