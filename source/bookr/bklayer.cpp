...
{
    struct T32FV32F2D vertices[2] = {{tx, ty, x, y, 0}, {tx + w, ty + h, x + w, y + h, 0}};
...
{
    struct T32FV32F2D vertices[2] = {{tx, ty, x, y, 0}, {tx + tw, ty + th, x + w, y + h, 0}};
    T32FV32F2D*       verts       = (T32FV32F2D*)FZScreen::getListMemory(2 * sizeof(struct T32FV32F2D));
    memcpy(verts, vertices, 2 * sizeof(struct T32FV32F2D));
    FZScreen::drawArray(FZ_SPRITES, FZ_TEXTURE_32BITF | FZ_VERTEX_32BITF | FZ_TRANSFORM_2D, 2, 0, verts);
}
...
{
    struct T32FV32F2D vertices[7 * 2] = {{tx, ty, x, y, 0},
                                         {tx + r, ty + r, x + r, y + r, 0},
...
{
    struct T32FV32F2D vertices[5 * 2] = {{tx, ty, x, y, 0},
                                         {tx + r, ty + r, x + r, y + r, 0},
...
{
    struct T32FV32F2D vertices[1 * 2] = {{tx + r, ty + r, x, y, 0}, {tx + r, ty + r, x + w, y + h, 0}};
