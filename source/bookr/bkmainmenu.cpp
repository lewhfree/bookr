...
string BKMainMenu::getPopupText() { return popupText; }

int BKMainMenu::getPopupMode() { return popupMode; }
    ...
    if (b[BKUser::controls.select] == 1) {
        ...
        if (selItem == MAIN_MENU_ITEM_ABOUT) {
            popupText = "Bookr - a document viewer for the Sony PSP.\nProgramming by Carlos, Edward and Chris.\nVisit http://bookr.sf.net for new versions.\nThis program is licensed under the terms of the GPL v2.\nUses "
                        "the MuPDF library under the terms \nof it's license.\nSeveral features by Paul Murray.";
...
{
    const int validMask = FZ_CTRL_UP | FZ_CTRL_RIGHT | FZ_CTRL_DOWN | FZ_CTRL_LEFT | FZ_CTRL_LTRIGGER | FZ_CTRL_RTRIGGER | FZ_CTRL_TRIANGLE | FZ_CTRL_CIRCLE | FZ_CTRL_CROSS | FZ_CTRL_SQUARE;
    if (captureButton) {
        ...
        switch (selItem) {
        case CONTROLS_MENU_ITEM_PREVIOUS_PAGE:
            BKUser::controls.previousPage = repsCode;
            break;
        case CONTROLS_MENU_ITEM_NEXT_PAGE:
            BKUser::controls.nextPage = repsCode;
            break;
        case CONTROLS_MENU_ITEM_PREVIOUS_10_PAGES:
            BKUser::controls.previous10Pages = repsCode;
            break;
        case CONTROLS_MENU_ITEM_NEXT_10_PAGES:
            BKUser::controls.next10Pages = repsCode;
            break;
        case CONTROLS_MENU_ITEM_SCREEN_UP:
            BKUser::controls.screenUp = repsCode;
            break;
        case CONTROLS_MENU_ITEM_SCREEN_DOWN:
            BKUser::controls.screenDown = repsCode;
            break;
        case CONTROLS_MENU_ITEM_SCREEN_LEFT:
            BKUser::controls.screenLeft = repsCode;
            break;
        case CONTROLS_MENU_ITEM_SCREEN_RIGHT:
            BKUser::controls.screenRight = repsCode;
            break;
        case CONTROLS_MENU_ITEM_ZOOM_IN:
            BKUser::controls.zoomIn = repsCode;
            break;
        case CONTROLS_MENU_ITEM_ZOOM_OUT:
            BKUser::controls.zoomOut = repsCode;
            break;
        }
        BKUser::save();
        buildControlMenu();
