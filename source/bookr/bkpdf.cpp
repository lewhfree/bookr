...
static const float zoomLevels[] = {0.25f, 0.5f, 0.75f, 0.90f, 1.0f, 1.1f, 1.2f, 1.3f, 1.4f, 1.5f, 1.6f, 1.7f, 1.8f, 1.9f, 2.0f, 2.25f, 2.5f, 2.75f, 3.0f, 3.5f, 4.0f, 5.0f, 7.5f, 10.0f, 16.0f};
...
static void pdfRenderFullPage(PDFContext* ctx)
{
    ...
    fullPageBuffer = pdfRenderTile(ctx, (int)ctx->page->mediabox.x0, (int)ctx->page->mediabox.y0, (int)w, (int)h, true);
    // precalc color shift
...
static void optimizeTransform(fz_transformnode* node) { optimizeNode(node->super.first); }
...
static void optimizeNode(fz_node* node)
{
    ...
    switch (node->kind) {
    case FZ_NIMAGE:
        optimizeImage((fz_imagenode*)node);
        break;
    case FZ_NMETA:
        optimizeMeta((fz_metanode*)node);
        break;
    case FZ_NOVER:
        optimizeOver((fz_overnode*)node);
        break;
    case FZ_NMASK:
        optimizeMask((fz_masknode*)node);
        break;
    case FZ_NBLEND:
        optimizeBlend((fz_blendnode*)node);
        break;
    case FZ_NTRANSFORM:
        optimizeTransform((fz_transformnode*)node);
        break;
    case FZ_NCOLOR:
        optimizeSolid((fz_solidnode*)node);
        break;
    case FZ_NPATH:
        optimizePath((fz_pathnode*)node);
        break;
    case FZ_NTEXT:
        optimizeText((fz_textnode*)node);
        break;
    case FZ_NSHADE:
        optimizeShade((fz_shadenode*)node);
        break;
    case FZ_NLINK:
        optimizeLink((fz_linknode*)node);
        break;
    }
}
...
bool BKPDF::isZoomable() { return true; }
...
int BKPDF::getCurrentZoomLevel() { return ctx->zoomLevel; }
...
bool BKPDF::hasZoomToFit() { return true; }
...
bool BKPDF::isBookmarkable() { return true; }
...
bool BKPDF::isPaginated() { return true; }

int BKPDF::getTotalPages() { return pdf_getpagecount(ctx->pages); }

int BKPDF::getCurrentPage() { return ctx->pageno; }
...
bool BKPDF::isRotable() { return true; }

int BKPDF::getRotation() { return ctx->rotateLevel; }
...
void BKPDF::getTitle(string& s) { s = title; }

void BKPDF::getType(string& s) { s = "PDF"; }

void BKPDF::getFileName(string& s) { s = fileName; }
...
int BKPDF::screenUp() { return prePan(0, -250); }

int BKPDF::screenDown() { return prePan(0, 250); }

int BKPDF::screenLeft() { return prePan(-460, 0); }

int BKPDF::screenRight() { return prePan(460, 0); }
