...
#ifdef MAC
static void* memalign(int t, int s) { return malloc(s); }
#else
#include <malloc.h>
#endif
    ...
    for (int i = 0; i < 256; ++i) {
        ...
        for (int row = 0; row < face->glyph->bitmap.rows; ++row) {
            for (int pixel = 0; pixel < face->glyph->bitmap.width; ++pixel) {
                fontTexture[(x + face->glyph->bitmap_left + pixel) + (y - face->glyph->bitmap_top + row) * image_width] = face->glyph->bitmap.buffer[pixel + row * face->glyph->bitmap.pitch];
            }
        }
        x += advance;
    }
...
int FZFont::getLineHeight() { return lineHeight; }

FZCharMetrics* FZFont::getMetrics() { return metrics; }
