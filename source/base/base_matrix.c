...
#define D3(a1, a2, a3, b1, b2, b3, c1, c2, c3) (a1 * D2(b2, b3, c2, c3)) - (b1 * D2(a2, a3, c2, c3)) + (c1 * D2(a2, a3, b2, b3))

    det = D3(M3(m, 0, 0), M3(m, 1, 0), M3(m, 2, 0), M3(m, 0, 1), M3(m, 1, 1), M3(m, 2, 1), M3(m, 0, 2), M3(m, 1, 2), M3(m, 2, 2));
    if (det == 0)
        det = 1.0;
    det = 1.0 / det;
...
fz_matrix fz_invertmatrix(fz_matrix src)
{
    fz_matrix dst;
...
int fz_isrectilinear(fz_matrix m) { return (fabs(m.b) < FLT_EPSILON && fabs(m.c) < FLT_EPSILON) || (fabs(m.a) < FLT_EPSILON && fabs(m.d) < FLT_EPSILON); }

float fz_matrixexpansion(fz_matrix m) { return sqrt(fabs(m.a * m.d - m.b * m.c)); }
