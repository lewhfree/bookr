...
void FZTexture::bind() { FZScreen::setBoundTexture((FZTexture*)this); }
...
FZTexture::FZTexture() : texImage(0) { glGenTextures(1, &textureObject); }

FZTexture::~FZTexture() { glDeleteTextures(1, &textureObject); }
...
bool FZTexture::initFromImage(FZTexture* texture, FZImage* image, bool buildMipmaps)
{
    ...
    if (buildMipmaps) {
        glGetError();
        gluBuild2DMipmaps(GL_TEXTURE_2D, texture->internalFormat, width, height, texture->textureFormat, texture->textureType, data);
        ...
        glGetError();
        glTexImage2D(GL_TEXTURE_2D, 0, texture->internalFormat, width, height, 0, texture->textureFormat, texture->textureType, data);
        GLenum er = glGetError();
        if (er != GL_NO_ERROR) {
            printf("FZTextureData2D::createFromImage: %s\n", gluErrorString(er));
        }
...
void FZTexture::filter(int min, int mag)
{
    ...
    case FZ_LINEAR:
        texMin = GL_LINEAR;
        break;
    case FZ_NEAREST_MIPMAP_NEAREST:
        texMin = GL_NEAREST_MIPMAP_NEAREST;
        break;
    case FZ_LINEAR_MIPMAP_NEAREST:
        texMin = GL_LINEAR_MIPMAP_NEAREST;
        break;
    case FZ_NEAREST_MIPMAP_LINEAR:
        texMin = GL_NEAREST_MIPMAP_LINEAR;
        break;
    case FZ_LINEAR_MIPMAP_LINEAR:
        texMin = GL_LINEAR_MIPMAP_LINEAR;
        break;
    }
