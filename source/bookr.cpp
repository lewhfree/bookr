    ...
    while (!exitApp) {
        ...
        case BK_CMD_INVOKE_PAGE_CHOOSER: {
            if (documentLayer && documentLayer->isPaginated()) {
                ps = BKPageChooser::create(documentLayer->getTotalPages(), documentLayer->getCurrentPage(), BK_CMD_OPEN_PAGE);
                layers.push_back(ps);
            }
        } break;
        ...
        case BK_CMD_SET_FONT: {
            ...
            if (!isTTF(s)) {
                layers.push_back(BKPopup::create(BKPOPUP_ERROR, "The selected file is not a valid TrueType font."));
                break;
            }
            // file ok
            bkLayersIt it(layers.end());
        ...
        case BK_CMD_MAINMENU_POPUP:
            layers.push_back(BKPopup::create(mm->getPopupMode(), mm->getPopupText()));
            break;
