...
void fz_dropblendnode(fz_blendnode* node) { fz_dropcolorspace(node->cs); }
...
void fz_droplinknode(fz_linknode* node) { fz_droptree(node->tree); }

fz_rect fz_boundlinknode(fz_linknode* node, fz_matrix ctm) { return fz_boundtree(node->tree, ctm); }
...
fz_rect fz_boundsolidnode(fz_solidnode* node, fz_matrix ctm) { return fz_infiniterect; }

void fz_dropsolidnode(fz_solidnode* node) { fz_dropcolorspace(node->cs); }
...
void fz_dropshadenode(fz_shadenode* node) { fz_dropshade(node->shade); }

fz_rect fz_boundshadenode(fz_shadenode* node, fz_matrix ctm) { return fz_boundshade(node->shade, ctm); }
