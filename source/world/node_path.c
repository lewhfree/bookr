...
static fz_error* growpath(fz_pathnode* path, int n)
{
    int        newcap;
    fz_pathel* newels;
...
fz_error* fz_moveto(fz_pathnode* path, float x, float y)
{
...
fz_error* fz_lineto(fz_pathnode* path, float x, float y)
{
    if (growpath(path, 3) != nil)
        return fz_outofmem;
...
fz_error* fz_curveto(fz_pathnode* path, float x1, float y1, float x2, float y2, float x3, float y3)
{
...
fz_error* fz_curvetov(fz_pathnode* path, float x2, float y2, float x3, float y3)
{
...
fz_error* fz_curvetoy(fz_pathnode* path, float x1, float y1, float x3, float y3) { return fz_curveto(path, x1, y1, x3, y3, x3, y3); }

fz_error* fz_closepath(fz_pathnode* path)
{
...
fz_error* fz_endpath(fz_pathnode* path, fz_pathkind paint, fz_stroke* stroke, fz_dash* dash)
{
    fz_pathel* newels;
...
fz_rect fz_boundpathnode(fz_pathnode* path, fz_matrix ctm)
{
    fz_point p;
...
void fz_debugpathnode(fz_pathnode* path)
{
    float x, y;
    int   i = 0;
    while (i < path->len) {
...
void fz_dropdash(fz_dash* dash) { fz_free(dash); }
