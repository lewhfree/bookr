...
void fz_setfontwmode(fz_font* font, int wmode) { font->wmode = wmode; }
...
void fz_setdefaulthmtx(fz_font* font, int w) { font->dhmtx.w = w; }
...
void fz_debugfont(fz_font* font)
{
    ...
    printf("font '%s' {\n", font->name);
    printf("  wmode %d\n", font->wmode);
    printf("  bbox [%d %d %d %d]\n", font->bbox.x0, font->bbox.y0, font->bbox.x1, font->bbox.y1);
    printf("  DW %d\n", font->dhmtx.w);
    ...
    if (font->wmode) {
        ...
        for (i = 0; i < font->nvmtx; i++)
            printf("    <%04x> <%04x> %d %d %d\n", font->vmtx[i].lo, font->vmtx[i].hi, font->vmtx[i].x, font->vmtx[i].y, font->vmtx[i].w);
        printf("  }\n");
    }
