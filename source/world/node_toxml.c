...
static void xmlpath(fz_pathnode* node, int level)
{
    ...
    if (node->paint == FZ_STROKE) {
        printf("<path fill=\"stroke\" cap=\"%d\" join=\"%d\" width=\"%g\" miter=\"%g\"", node->linecap, node->linejoin, node->linewidth, node->miterlimit);
        if (node->dash) {
...
static void xmlnode(fz_node* node, int level)
{
    ...
    switch (node->kind) {
    case FZ_NMETA:
        xmlmeta((fz_metanode*)node, level);
        break;
    case FZ_NOVER:
...
void fz_debugnode(fz_node* node) { xmlnode(node, 0); }
