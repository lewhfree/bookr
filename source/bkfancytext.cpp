...
static inline bool isBlank(int c) { return c == 32 || c == 10 || c == 9; }
...
struct BKRunsIterator {
    ...
    int                  lineBreakMark;
    int                  globalPos;
    bool                 end() { return currentRun >= maxRuns || ((currentRun == maxRuns - 1) && currentChar >= runs[currentRun].n); }
    ...
    BKRunsIterator(const BKRunsIterator& s) : runs(s.runs), currentRun(s.currentRun), currentChar(s.currentRun), maxRuns(s.currentRun), lineBreakMark(s.lineBreakMark), globalPos(s.globalPos) {}
};
...
struct BKStrIt {
    ...
    { // does NOT advance the iteration
        char* q = p;
        int   j = i;
        while (*s != 0 && j < n) {
...
{
    if (lastFontSize != BKUser::options.txtSize || lastFontFace != BKUser::options.txtFont || lastHeightPct != BKUser::options.txtHeightPct // should be able to just resize view here
        || lastWrapCR != BKUser::options.txtWrapCR)
        return BK_CMD_RELOAD;
    return 0;
}

int BKFancyText::resume() { return 0; }
...
bool BKFancyText::isPaginated() { return true; }

int BKFancyText::getTotalPages() { return totalPages; }

int BKFancyText::getCurrentPage() { return (topLine / linesPerPage) + 1; }
...
int BKFancyText::screenUp() { return setCurrentPage(getCurrentPage() - 1); }

int BKFancyText::screenDown() { return setCurrentPage(getCurrentPage() + 1); }

int BKFancyText::screenLeft() { return setCurrentPage(getCurrentPage() - 10); }

int BKFancyText::screenRight() { return setCurrentPage(getCurrentPage() + 10); }

bool BKFancyText::isRotable() { return true; }

int BKFancyText::getRotation() { return rotation; }
...
bool BKFancyText::isBookmarkable() { return true; }
...
bool BKFancyText::isZoomable() { return false; }

void BKFancyText::getZoomLevels(vector<BKDocument::ZoomLevel>& v) {}

int BKFancyText::getCurrentZoomLevel() { return 0; }

int BKFancyText::setZoomLevel(int l) { return 0; }

bool BKFancyText::hasZoomToFit() { return false; }

int BKFancyText::setZoomToFitWidth() { return 0; }

int BKFancyText::setZoomToFitHeight() { return 0; }
