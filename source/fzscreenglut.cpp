...
static void updateReps()
{
    ...
    if (keyState & FZ_CTRL_SELECT)
        breps[FZ_REPS_SELECT]++;
    else
        breps[FZ_REPS_SELECT] = 0;
    if (keyState & FZ_CTRL_START)
        breps[FZ_REPS_START]++;
    else
        breps[FZ_REPS_START] = 0;
    if (keyState & FZ_CTRL_UP)
        breps[FZ_REPS_UP]++;
    else
        breps[FZ_REPS_UP] = 0;
    if (keyState & FZ_CTRL_RIGHT)
        breps[FZ_REPS_RIGHT]++;
    else
        breps[FZ_REPS_RIGHT] = 0;
    if (keyState & FZ_CTRL_DOWN)
        breps[FZ_REPS_DOWN]++;
    else
        breps[FZ_REPS_DOWN] = 0;
    if (keyState & FZ_CTRL_LEFT)
        breps[FZ_REPS_LEFT]++;
    else
        breps[FZ_REPS_LEFT] = 0;
    if (keyState & FZ_CTRL_LTRIGGER)
        breps[FZ_REPS_LTRIGGER]++;
    else
        breps[FZ_REPS_LTRIGGER] = 0;
    if (keyState & FZ_CTRL_RTRIGGER)
        breps[FZ_REPS_RTRIGGER]++;
    else
        breps[FZ_REPS_RTRIGGER] = 0;
    if (keyState & FZ_CTRL_TRIANGLE)
        breps[FZ_REPS_TRIANGLE]++;
    else
        breps[FZ_REPS_TRIANGLE] = 0;
    if (keyState & FZ_CTRL_CIRCLE)
        breps[FZ_REPS_CIRCLE]++;
    else
        breps[FZ_REPS_CIRCLE] = 0;
    if (keyState & FZ_CTRL_CROSS)
        breps[FZ_REPS_CROSS]++;
    else
        breps[FZ_REPS_CROSS] = 0;
    if (keyState & FZ_CTRL_SQUARE)
        breps[FZ_REPS_SQUARE]++;
    else
        breps[FZ_REPS_SQUARE] = 0;
    if (keyState & FZ_CTRL_HOME)
        breps[FZ_REPS_HOME]++;
    else
        breps[FZ_REPS_HOME] = 0;
    if (keyState & FZ_CTRL_HOLD)
        breps[FZ_REPS_HOLD]++;
    else
        breps[FZ_REPS_HOLD] = 0;
    if (keyState & FZ_CTRL_NOTE)
        breps[FZ_REPS_NOTE]++;
    else
        breps[FZ_REPS_NOTE] = 0;
}
...
static void keyboard(unsigned char key, int x, int y)
{
    ...
    case 'k':
        ...
        break;
    case 'l':
        keyState |= FZ_CTRL_CROSS;
        break;
    case 'o':
        keyState |= FZ_CTRL_TRIANGLE;
        break;
    case 'p':
        keyState |= FZ_CTRL_CIRCLE;
        break;
    case 'v':
        keyState |= FZ_CTRL_SELECT;
        break;
    case 'b':
        keyState |= FZ_CTRL_START;
        break;
    case 'x':
        keyState |= FZ_CTRL_LTRIGGER;
        break;
    case 'c':
        keyState |= FZ_CTRL_RTRIGGER;
        break;
    }
}
...
void FZScreen::setBoundTexture(FZTexture* t) { boundTexture = t; }

int FZScreen::setupCallbacks() { return 0; }

void FZScreen::close() {}

void FZScreen::exit() { ::exit(0); }

void FZScreen::setupCtrl() { resetReps(); }
...
void FZScreen::resetReps() { stickyKeys = true; }

int* FZScreen::ctrlReps() { return breps; }

void FZScreen::startDirectList() { topList = 0; }
...
void FZScreen::swapBuffers() { glutSwapBuffers(); }
...
{
    glClearColor(((float)(c & 0xff)) / 255.0f, ((float)((c & 0xff00) >> 8)) / 255.0f, ((float)((c & 0xff0000) >> 16)) / 255.0f, ((float)((c & 0xff000000) >> 24)) / 255.0f);
    int m = 0;
...
static int toGLBlendMode(int v)
{
    ...
    case FZ_SRC_COLOR:
        r = GL_SRC_COLOR;
        break;
    case FZ_ONE_MINUS_SRC_COLOR:
        r = GL_ONE_MINUS_SRC_COLOR;
        break;
    case FZ_SRC_ALPHA:
        r = GL_SRC_ALPHA;
        break;
    case FZ_ONE_MINUS_SRC_ALPHA:
...
static int toGLState(int v)
{
    ...
    case FZ_DEPTH_TEST:
        r = GL_DEPTH_TEST;
        break;
    case FZ_SCISSOR_TEST:
        r = GL_SCISSOR_TEST;
        break;
    case FZ_STENCIL_TEST:
        r = GL_STENCIL_TEST;
        break;
    case FZ_BLEND:
        r = GL_BLEND;
...
void FZScreen::enable(int m) { glEnable(toGLState(m)); }

void FZScreen::disable(int m) { glDisable(toGLState(m)); }

void FZScreen::dcacheWritebackAll() {}

void FZScreen::shadeModel(int mode) {}

void FZScreen::ambientColor(unsigned int c) { glColor4ub(c & 0xff, (c & 0xff00) >> 8, (c & 0xff0000) >> 16, (c & 0xff000000) >> 24); }
...
void          FZScreen::copyImage(int psm, int sx, int sy, int width, int height, int srcw, void* src, int dx, int dy, int destw, void* dest)
{
    ...
    glTexSubImage2D(GL_TEXTURE_2D, 0, dx, dy, width, height, GL_RGBA, GL_UNSIGNED_BYTE, src);
...
void* FZScreen::framebuffer() { return NULL; }
...
*/
char* FZScreen::basePath() { return psp_full_path; }

int FZScreen::getSuspendSerial() { return powerSerial; }
...
int FZScreen::getBattery() { return 100; }
