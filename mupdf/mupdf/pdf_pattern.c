...
fz_error* pdf_loadpattern(pdf_pattern** patp, pdf_xref* xref, fz_obj* dict, fz_obj* stmref)
{
    fz_error*    error;
    pdf_pattern* pat;
    fz_stream*   stm;
    ...
    pdf_logrsrc("bbox [%g %g %g %g]\n", pat->bbox.x0, pat->bbox.y0, pat->bbox.x1, pat->bbox.y1);
    ...
    pdf_logrsrc("matrix [%g %g %g %g %g %g]\n", pat->matrix.a, pat->matrix.b, pat->matrix.c, pat->matrix.d, pat->matrix.e, pat->matrix.f);
