...
static void t3dropfont(fz_font* font)
{
    int       i;
...
static fz_error* t3render(fz_glyph* glyph, fz_font* fzfont, int cid, fz_matrix trm)
{
...
static fz_error* loadcharproc(fz_tree** treep, pdf_xref* xref, fz_obj* rdb, fz_obj* stmref)
{
    fz_error*  error;
    pdf_csi*   csi;
    fz_stream* stm;
...
fz_error* pdf_loadtype3font(pdf_font** fontp, pdf_xref* xref, fz_obj* dict, fz_obj* ref)
{
    fz_error* error;
    char      buf[256];
    char*     estrings[256];
    ...
    pdf_logfont("matrix [%g %g %g %g %g %g]\n", font->matrix.a, font->matrix.b, font->matrix.c, font->matrix.d, font->matrix.e, font->matrix.f);
