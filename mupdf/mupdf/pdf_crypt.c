...
static const unsigned char padding[32] = {0x28, 0xbf, 0x4e, 0x5e, 0x4e, 0x75, 0x8a, 0x41, 0x64, 0x00, 0x4e, 0x56, 0xff, 0xfa, 0x01, 0x08,
...
{
    if (crypt->encrypt)
        fz_dropobj(crypt->encrypt);
    if (crypt->id)
        fz_dropobj(crypt->id);
    fz_free(crypt);
}

static void createobjkey(pdf_crypt* crypt, unsigned oid, unsigned gid, unsigned char* key)
{
 ...
 */
static void createkey(pdf_crypt* crypt, char* userpw, int pwlen)
{
 ...
 */
static void createowner(pdf_crypt* crypt, char* userpw, char* ownerpw)
{
    unsigned char buf[32];
 ...
 */
static void createuser(pdf_crypt* crypt, char* userpw, int pwlen)
{
    unsigned char key[16];
    fz_arc4       arc4;
    fz_md5        md5;
...
fz_error* pdf_newencrypt(pdf_crypt** cp, char* userpw, char* ownerpw, int p, int n, fz_obj* id)
{
    ...
    error = fz_packobj(&crypt->encrypt,
                       ...
                       "/P %i "
                       "/Length %i >>",
                       crypt->r == 2 ? 1 : 2, crypt->r, crypt->o, 32, crypt->u, 32, crypt->p, crypt->n * 8);
    if (error) {
        pdf_dropcrypt(crypt);
        return error;
    }
...
fz_error* pdf_setuserpassword(pdf_crypt* crypt, char* userpw, int pwlen)
{
...
fz_error* pdf_setownerpassword(pdf_crypt* crypt, char* ownerpw, int pwlen)
{
    unsigned char buf[32];
    unsigned char key[16];
    fz_arc4       arc4;
