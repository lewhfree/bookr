...
enum { MAXN = FZ_MAXCOLORS, MAXM = FZ_MAXCOLORS, MAXK = 32 };

typedef struct psobj_s psobj;

enum { SAMPLE = 0, EXPONENTIAL = 2, STITCHING = 3, POSTSCRIPT = 4 };
...
#define LERP(x, xmin, xmax, ymin, ymax) (ymin) + ((x) - (xmin)) * ((ymax) - (ymin)) / ((xmax) - (xmin))

#define SAFE_PUSHINT(st, a)       \
    {                             \
        error = pspushint(st, a); \
        if (error)                \
            goto cleanup;         \
    }
#define SAFE_PUSHREAL(st, a)       \
    {                              \
        error = pspushreal(st, a); \
        if (error)                 \
            goto cleanup;          \
    }
#define SAFE_PUSHBOOL(st, a)       \
    {                              \
        error = pspushbool(st, a); \
        if (error)                 \
            goto cleanup;          \
    }
...
static char* psopnames[] = {
    "abs",   "add", "and", "atan", "bitshift", "ceiling", "copy", "cos", "cvi", "cvr", "div", "dup", "eq",   "exch",  "exp", "false", "floor", "ge",   "gt",       "idiv",
    "index", "le",  "ln",  "log",  "lt",       "mod",     "mul",  "ne",  "neg", "not", "or",  "pop", "roll", "round", "sin", "sqrt",  "sub",   "true", "truncate", "xor", /* "if", "ifelse", "return" */
};
...
static int pscheckoverflow(psstack* st, int n) { return st->sp >= n; }

static int pscheckunderflow(psstack* st) { return st->sp != PSSTACKSIZE; }

static int pschecktype(psstack* st, unsigned short t1, unsigned short t2) { return (st->stack[st->sp].type == t1 || st->stack[st->sp].type == t2); }

static fz_error* pspushbool(psstack* st, int booln)
{
...
static fz_error* pspushint(psstack* st, int intg)
{
    if (pscheckoverflow(st, 1)) {
...
static fz_error* pspopbool(psstack* st, int* booln)
{
...
static fz_error* pspopint(psstack* st, int* intg)
{
...
static fz_error* pspopnum(psstack* st, float* real)
{
    if (pscheckunderflow(st) && pschecktype(st, PSINT, PSREAL)) {
        float ret;
        ret = (st->stack[st->sp].type == PSINT) ? (float)st->stack[st->sp].u.i : st->stack[st->sp].u.f;
        ++st->sp;
        *real = ret;
    } else if (pscheckunderflow(st))
        return fz_stackunderflow;
...
static int pstopisint(psstack* st) { return st->sp < PSSTACKSIZE && st->stack[st->sp].type == PSINT; }

static int pstoptwoareints(psstack* st) { return st->sp < PSSTACKSIZE - 1 && st->stack[st->sp].type == PSINT && st->stack[st->sp + 1].type == PSINT; }

static int pstopisreal(psstack* st) { return st->sp < PSSTACKSIZE && st->stack[st->sp].type == PSREAL; }

static int pstoptwoarenums(psstack* st)
{
    return st->sp < PSSTACKSIZE - 1 && (st->stack[st->sp].type == PSINT || st->stack[st->sp].type == PSREAL) && (st->stack[st->sp + 1].type == PSINT || st->stack[st->sp + 1].type == PSREAL);
}

static fz_error* pscopy(psstack* st, int n)
{
    int i;
...
static void psroll(psstack* st, int n, int j)
{
    psobj obj;
    int   i, k;
...
static fz_error* psindex(psstack* st, int i)
{
    if (!pscheckoverflow(st, 1)) {
        return fz_stackoverflow;
    }
...
static fz_error* pspop(psstack* st)
{
    if (!pscheckoverflow(st, 1)) {
        return fz_stackoverflow;
    }
...
static fz_error* resizecode(pdf_function* func, int newsize)
{
...
static fz_error* parsecode(pdf_function* func, fz_stream* stream, int* codeptr)
{
    fz_error* error = nil;
    char      buf[64];
...
static fz_error* loadpostscriptfunc(pdf_function* func, pdf_xref* xref, fz_obj* dict, int oid, int gen)
{
    fz_error*  error = nil;
    fz_stream* stream;
    int        codeptr;
...
static fz_error* evalpostscriptfunc(pdf_function* func, psstack* st, int codeptr)
{
    fz_error* error = nil;
    int       i1, i2;
    float     r1, r2;
...
static fz_error* loadsamplefunc(pdf_function* func, pdf_xref* xref, fz_obj* dict, int oid, int gen)
{
    fz_error*  error = nil;
    fz_stream* stream;
    fz_obj*    obj;
...
static fz_error* evalsamplefunc(pdf_function* func, float* in, float* out)
{
    float  x;
    int    e[2][MAXM];
    float  efrac[MAXM];
    ...
    for (i = 0; i < func->n; i++) {
        ...
        out[i] = LERP(s0[0], 0, (1 << func->u.sa.bps) - 1, func->u.sa.decode[i][0], func->u.sa.decode[i][1]);
...
static fz_error* loadexponentialfunc(pdf_function* func, fz_obj* dict)
{
    fz_error* error = nil;
    fz_obj*   obj;
    int       i;
...
static fz_error* evalexponentialfunc(pdf_function* func, float in, float* out)
{
    fz_error* error = nil;
    float     x     = in;
    float     tmp;
...
static fz_error* loadstitchingfunc(pdf_function* func, pdf_xref* xref, fz_obj* dict)
{
    ...
    obj = fz_dictgets(dict, "Bounds");
    {
        ...
        if (k != 1 && (func->domain[0][0] >= func->u.st.bounds[0] || func->domain[0][1] <= func->u.st.bounds[k - 2]))
            goto cleanup;
...
static fz_error* evalstitchingfunc(pdf_function* func, float in, float* out)
{
    fz_error* error = nil;
    float     low, high;
    int       k      = func->u.st.k;
...
fz_error* pdf_loadfunction(pdf_function** funcp, pdf_xref* xref, fz_obj* ref)
{
    fz_error*     error;
    pdf_function* func;
    fz_obj*       dict;
