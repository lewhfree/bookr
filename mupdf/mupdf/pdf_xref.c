...
void pdf_closexref(pdf_xref* xref)
{
...
fz_error* pdf_initxref(pdf_xref* xref)
{
...
void pdf_flushxref(pdf_xref* xref, int force)
{
    int i;
...
void pdf_debugxref(pdf_xref* xref)
{
    int i;
    printf("xref\n0 %d\n", xref->len);
    for (i = 0; i < xref->len; i++) {
        printf("%010d %05d %c | %d %c%c\n", xref->table[i].ofs, xref->table[i].gen, xref->table[i].type, xref->table[i].obj ? xref->table[i].obj->refs : 0, xref->table[i].stmofs ? 'f' : '-',
...
fz_error* pdf_allocobject(pdf_xref* xref, int* oidp, int* genp)
{
    pdf_xrefentry* x;
    int            prev, next;
    int            oid = 0;
...
fz_error* pdf_deleteobject(pdf_xref* xref, int oid, int gen)
{
    pdf_xrefentry* x;
    int            prev;
...
fz_error* pdf_updateobject(pdf_xref* xref, int oid, int gen, fz_obj* obj)
{
    pdf_xrefentry* x;
...
fz_error* pdf_updatestream(pdf_xref* xref, int oid, int gen, fz_buffer* stm)
{
    pdf_xrefentry* x;
...
fz_error* pdf_cacheobject(pdf_xref* xref, int oid, int gen)
{
...
fz_error* pdf_loadobject(fz_obj** objp, pdf_xref* xref, int oid, int gen)
{
    fz_error* error;
...
fz_error* pdf_loadindirect(fz_obj** objp, pdf_xref* xref, fz_obj* ref)
{
    assert(ref != nil);
    return pdf_loadobject(objp, xref, fz_tonum(ref), fz_togen(ref));
}

fz_error* pdf_resolve(fz_obj** objp, pdf_xref* xref)
{
