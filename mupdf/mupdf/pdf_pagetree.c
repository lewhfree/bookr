...
static fz_error* loadpagetree(pdf_xref* xref, pdf_pagetree* pages, struct stuff inherit, fz_obj* obj, fz_obj* ref)
{
    fz_error* error;
    fz_obj*   type;
    fz_obj*   kids;
    ...
    if (strcmp(fz_toname(type), "Page") == 0) {
        ...
        if (inh)
            inherit.resources = inh;

        inh = fz_dictgets(obj, "MediaBox");
        if (inh)
            inherit.mediabox = inh;
...
int pdf_getpagecount(pdf_pagetree* pages) { return pages->count; }
