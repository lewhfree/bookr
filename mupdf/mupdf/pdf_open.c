...
static fz_error* loadversion(pdf_xref* xref)
{
    char buf[20];
    int  n;
...
static fz_error* readstartxref(pdf_xref* xref)
{
    char buf[1024];
    int  t, n;
    int  i;
...
static fz_error* readoldtrailer(pdf_xref* xref, char* buf, int cap)
{
    int   ofs, len;
    char* s;
    int   n;
...
static fz_error* readnewtrailer(pdf_xref* xref, char* buf, int cap)
{
    pdf_logxref("load new xref format trailer\n");
    return pdf_parseindobj(&xref->trailer, xref->file, buf, cap, nil, nil, nil);
}

static fz_error* readtrailer(pdf_xref* xref, char* buf, int cap)
{
    int n;
    int c;
...
static fz_error* readoldxref(fz_obj** trailerp, pdf_xref* xref, char* buf, int cap)
{
    int   ofs, len;
    char* s;
    int   n;
...
static fz_error* readnewxref(fz_obj** trailerp, pdf_xref* xref, char* buf, int cap)
{
    fz_error*  error;
    fz_stream* stm;
    fz_obj*    trailer;
...
static fz_error* readxref(fz_obj** trailerp, pdf_xref* xref, int ofs, char* buf, int cap)
{
    int n;
    int c;
...
static fz_error* readxrefsections(pdf_xref* xref, int ofs, char* buf, int cap)
{
    fz_error* error;
    fz_obj*   trailer;
    fz_obj*   prev;
...
fz_error* pdf_loadxref(pdf_xref* xref, char* filename)
{
    fz_error* error;
    fz_obj*   size;
    int       i;
