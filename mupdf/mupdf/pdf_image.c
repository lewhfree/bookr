...
static void loadcolorkey(int* colorkey, int bpc, int indexed, fz_obj* obj)
{
    int scale = 1;
    int i;
...
void             bk_pdf_resetbufferssize() { image_buffers_size = 0; }

int bk_pdf_overflowbuffers() { return image_buffers_size >= image_buffers_size_max ? 1 : 0; }

/* TODO error cleanup */
fz_error* pdf_loadimage(pdf_image** imgp, pdf_xref* xref, fz_obj* dict, fz_obj* ref)
{
    fz_error*  error;
    pdf_image* img;
    pdf_image* mask;
...
static void maskcolorkey(fz_pixmap* pix, int* colorkey)
{
...
static void maskcolorkeyindexed(fz_pixmap* ind, fz_pixmap* pix, int* colorkey)
{
...
fz_error* pdf_loadtile(fz_image* img, fz_pixmap* tile)
{
    ...
    switch (src->bpc) {
    case 1:
        tilefunc = fz_loadtile1;
        break;
    case 2:
        tilefunc = fz_loadtile2;
        break;
    case 4:
        tilefunc = fz_loadtile4;
        break;
    case 8:
        tilefunc = fz_loadtile8;
        break;
    default:
    ...
    if (src->indexed) {
        ...
        tilefunc(src->samples->rp, src->stride, tmp->samples, tmp->w, tmp->w, tmp->h, 0);
        ...
        for (y = 0; y < tile->h; y++) {
            for (x = 0; x < tile->w; x++) {
                ...
                i                                          = CLAMP(i, 0, src->indexed->high);
                for (k = 0; k < src->indexed->base->n; k++) {
                    tile->samples[(y * tile->w + x) * tile->n + k + 1] = src->indexed->lookup[i * src->indexed->base->n + k];
                }
            }
        }
        ...
        tilefunc(src->samples->rp, src->stride, tile->samples, tile->w * tile->n, img->w * (img->n + img->a), img->h, img->a ? 0 : img->n);
        if (src->usecolorkey)
