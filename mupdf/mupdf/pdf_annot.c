...
static fz_error* loadcomment(pdf_comment** commentp, pdf_xref* xref, fz_obj* dict) { return nil; }

fz_error* pdf_newlink(pdf_link** linkp, fz_rect bbox, fz_obj* dest)
{
    pdf_link* link;
...
static fz_obj* resolvedest(pdf_xref* xref, fz_obj* dest)
{
    if (fz_isname(dest)) {
...
fz_error* pdf_loadannots(pdf_comment** cp, pdf_link** lp, pdf_xref* xref, fz_obj* annots)
{
    fz_error*    error;
    pdf_comment* comment;
    pdf_link*    link;
