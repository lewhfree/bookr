...
static int cmprange(const void* va, const void* vb) { return ((const pdf_range*)va)->low - ((const pdf_range*)vb)->low; }
...
pdf_cmap* pdf_getusecmap(pdf_cmap* cmap) { return cmap->usecmap; }
...
int pdf_getwmode(pdf_cmap* cmap) { return cmap->wmode; }

void pdf_setwmode(pdf_cmap* cmap, int wmode) { cmap->wmode = wmode; }
 ...
 */
static fz_error* addtable(pdf_cmap* cmap, int value)
{
 ...
 * Add a range.
 */
static fz_error* addrange(pdf_cmap* cmap, int low, int high, int flag, int offset)
{
 ...
 */
fz_error* pdf_maprangetotable(pdf_cmap* cmap, int low, int* table, int len)
{
    fz_error* error;
    int       offset;
    int       high;
 ...
 */
fz_error* pdf_maprangetorange(pdf_cmap* cmap, int low, int high, int offset) { return addrange(cmap, low, high, high - low == 0 ? SINGLE : RANGE, offset); }

/*
 * Add a single one-to-many mapping.
 */
fz_error* pdf_maponetomany(pdf_cmap* cmap, int low, int* values, int len)
{
    fz_error* error;
    int       offset;
    int       i;
 ...
 */
unsigned char* pdf_decodecmap(pdf_cmap* cmap, unsigned char* buf, int* cpt)
{
    int i, k;
...
enum { TUSECMAP = PDF_NTOKENS, TBEGINCODESPACERANGE, TENDCODESPACERANGE, TBEGINBFCHAR, TENDBFCHAR, TBEGINBFRANGE, TENDBFRANGE, TBEGINCIDCHAR, TENDCIDCHAR, TBEGINCIDRANGE, TENDCIDRANGE };

static int tokenfromkeyword(char* key)
{
    if (!strcmp(key, "usecmap"))
        return TUSECMAP;
    if (!strcmp(key, "begincodespacerange"))
        return TBEGINCODESPACERANGE;
    if (!strcmp(key, "endcodespacerange"))
        return TENDCODESPACERANGE;
    if (!strcmp(key, "beginbfchar"))
        return TBEGINBFCHAR;
    if (!strcmp(key, "endbfchar"))
        return TENDBFCHAR;
    if (!strcmp(key, "beginbfrange"))
        return TBEGINBFRANGE;
    if (!strcmp(key, "endbfrange"))
        return TENDBFRANGE;
    if (!strcmp(key, "begincidchar"))
        return TBEGINCIDCHAR;
    if (!strcmp(key, "endcidchar"))
        return TENDCIDCHAR;
    if (!strcmp(key, "begincidrange"))
        return TBEGINCIDRANGE;
    if (!strcmp(key, "endcidrange"))
        return TENDCIDRANGE;
    return PDF_TKEYWORD;
}
 ...
 */
fz_error* pdf_loadembeddedcmap(pdf_cmap** cmapp, pdf_xref* xref, fz_obj* stmref)
{
