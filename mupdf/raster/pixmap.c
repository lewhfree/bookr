...
fz_error* fz_newpixmap(fz_pixmap** pixp, int x, int y, int w, int h, int n)
{
    fz_pixmap* pix;
...
fz_error* fz_newpixmapwithrect(fz_pixmap** pixp, fz_irect r, int n) { return fz_newpixmap(pixp, r.x0, r.y0, r.x1 - r.x0, r.y1 - r.y0, n); }
...
void fz_clearpixmap(fz_pixmap* pix) { memset(pix->samples, 0, pix->w * pix->h * pix->n * sizeof(fz_sample)); }
...
void fz_debugpixmap(fz_pixmap* pix)
{
    if (pix->n == 4) {
        int   x, y;
