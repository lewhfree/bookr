...
static fz_error* rendertransform(fz_renderer* gc, fz_transformnode* transform, fz_matrix ctm)
{
    fz_error* error;
    DEBUG("transform [%g %g %g %g %g %g]\n", transform->m.a, transform->m.b, transform->m.c, transform->m.d, transform->m.e, transform->m.f);
    DEBUG("{\n");
...
static fz_error* rendersolid(fz_renderer* gc, fz_solidnode* solid, fz_matrix ctm)
{
    fz_error*      error;
    float          rgb[3];
    unsigned char* p;
...
static fz_error* renderpath(fz_renderer* gc, fz_pathnode* path, fz_matrix ctm)
{
    fz_error* error;
    float     flatness;
    fz_irect  gbox;
    ...
    if (gc->flag & FRGB) {
        return fz_scanconvert(gc->gel, gc->ael, path->paint == FZ_EOFILL, clip, gc->over, gc->rgb, 1);
    } else if (gc->flag & FOVER) {
        return fz_scanconvert(gc->gel, gc->ael, path->paint == FZ_EOFILL, clip, gc->over, nil, 1);
    } else {
        error = fz_newpixmapwithrect(&gc->dest, clip, 1);
        if (error)
            return error;
        fz_clearpixmap(gc->dest);
        return fz_scanconvert(gc->gel, gc->ael, path->paint == FZ_EOFILL, clip, gc->dest, nil, 0);
    }
}
...
static fz_error* rendertext(fz_renderer* gc, fz_textnode* text, fz_matrix ctm)
{
    fz_error* error;
    fz_irect  tbox;
    fz_irect  clip;
    ...
    DEBUG("text %s n=%d [%g %g %g %g];\n", text->font->name, text->len, text->trm.a, text->trm.b, text->trm.c, text->trm.d);
...
static inline void calcimagescale(fz_matrix ctm, int w, int h, int* odx, int* ody)
{
    float sx, sy;
    int   dx, dy;
...
static fz_error* renderimage(fz_renderer* gc, fz_imagenode* node, fz_matrix ctm)
{
    fz_error*  error;
...
static fz_error* rendershade(fz_renderer* gc, fz_shadenode* node, fz_matrix ctm)
{
    fz_error* error;
    fz_irect  bbox;
...
static void blendover(fz_renderer* gc, fz_pixmap* src, fz_pixmap* dst)
{
    unsigned char *sp, *dp;
    fz_irect       sr, dr;
    int            x, y, w, h;
...
static void blendmask(fz_renderer* gc, fz_pixmap* src, fz_pixmap* msk, fz_pixmap* dst, int over)
{
...
static fz_error* renderover(fz_renderer* gc, fz_overnode* over, fz_matrix ctm)
{
    fz_error* error;
    fz_node*  child;
    int       cluster = 0;
...
static fz_error* rendermask(fz_renderer* gc, fz_masknode* mask, fz_matrix ctm)
{
    fz_error*  error;
    int        oldmaskonly;
    fz_pixmap* oldover;
...
fz_error* fz_rendertree(fz_pixmap** outp, fz_renderer* gc, fz_tree* tree, fz_matrix ctm, fz_irect bbox, int white)
{
    fz_error* error;
    ...
    DEBUG("tree %d [%d %d %d %d]\n{\n", gc->maskonly ? 1 : 4, bbox.x0, bbox.y0, bbox.x1, bbox.y1);
