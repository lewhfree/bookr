...
static int clippoly(float src[MAXV][MAXN], float dst[MAXV][MAXN], int len, int n, float val, int isy, int ismax)
{
    float cv1[MAXN];
    float cv2[MAXN];
    int   v1, v2, cp;
...
static inline void drawscan(fz_pixmap* pix, int y, int x1, int x2, int* v1, int* v2, int n)
{
...
static inline int findnext(int gel[MAXV][MAXN], int len, int a, int* s, int* e, int d)
{
    int b;
...
static inline void loadedge(int gel[MAXV][MAXN], int s, int e, int* ael, int* del, int n)
{
    int swp, k, dy;
...
static inline void stepedge(int* ael, int* del, int n)
{
    int k;
    ael[0] += del[0];
    for (k = 2; k < n; k++)
...
void fz_drawtriangle(fz_pixmap* pix, float* av, float* bv, float* cv, int n)
{
...
fz_error* fz_rendershade(fz_shade* shade, fz_matrix ctm, fz_colorspace* destcs, fz_pixmap* dest)
{
    ...
    if (shade->usefunction) {
        ...
        n     = 2 + shade->cs->n;
        error = fz_newpixmap(&temp, dest->x, dest->y, dest->w, dest->h, shade->cs->n + 1);
        if (error)
            return error;
    } else {
        n    = 2 + shade->cs->n;
