...
static void stdfree(fz_memorycontext* mem, void* p) { free(p); }

static fz_memorycontext  defmem = {stdmalloc, stdrealloc, stdfree};
static fz_memorycontext* curmem = &defmem;

fz_error fz_koutofmem = {-1, {"out of memory"}, {"<malloc>"}, {"memory.c"}, 0};

fz_memorycontext* fz_currentmemorycontext() { return curmem; }

void fz_setmemorycontext(fz_memorycontext* mem) { curmem = mem; }
