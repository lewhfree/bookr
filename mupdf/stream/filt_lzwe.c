...
static void clearhash(fz_lzwe* lzw)
{
    int i;
    for (i = 0; i < HSIZE; i++)
        lzw->table[i].hash = -1;
...
fz_error* fz_newlzwe(fz_filter** fp, fz_obj* params)
{
    ...
    if (params) {
        ...
        if (obj)
            lzw->earlychange = fz_toint(obj) != 0;
    }
...
void fz_droplzwe(fz_filter* filter) {}

static void putcode(fz_lzwe* lzw, fz_buffer* out, int code)
{
...
static fz_error* compress(fz_lzwe* lzw, fz_buffer* in, fz_buffer* out)
{
    if (lzw->resume) {
        lzw->resume = 0;
        goto resume;
...
fz_error* fz_processlzwe(fz_filter* filter, fz_buffer* in, fz_buffer* out)
{
