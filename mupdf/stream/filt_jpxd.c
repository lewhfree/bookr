...
fz_error* fz_newjpxd(fz_filter** fp, fz_obj* params)
{
    int err;
...
void fz_dropjpxd(fz_filter* filter)
{
    fz_jpxd* d = (fz_jpxd*)filter;
    if (d->stream)
        jas_stream_close(d->stream);
    if (d->image)
        jas_image_destroy(d->image);
}

fz_error* fz_processjpxd(fz_filter* filter, fz_buffer* in, fz_buffer* out)
{
    fz_jpxd* d = (fz_jpxd*)filter;
    int      n, bpc, w, h;
    int      i, x, y;
...
decode:
    ...
    fprintf(stderr, "P%c\n# JPX %d x %d n=%d bpc=%d colorspace=%04x %s\n%d %d\n%d\n", jas_image_numcmpts(d->image) == 1 ? '5' : '6',
            ...
            jas_image_width(d->image), jas_image_height(d->image), jas_image_numcmpts(d->image), jas_image_cmptprec(d->image, 0), jas_image_clrspc(d->image), colorspacename(jas_image_clrspc(d->image)),

            jas_image_width(d->image), jas_image_height(d->image), (1 << jas_image_cmptprec(d->image, 0)) - 1);
