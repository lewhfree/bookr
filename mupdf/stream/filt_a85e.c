...
fz_error* fz_newa85e(fz_filter** fp, fz_obj* params)
{
...
void fz_dropa85e(fz_filter* f) {}

fz_error* fz_processa85e(fz_filter* filter, fz_buffer* in, fz_buffer* out)
{
    ...
    while (1) {
        ...
        if (in->rp + 4 <= in->wp) {
            word = (in->rp[0] << 24) | (in->rp[1] << 16) | (in->rp[2] << 8) | (in->rp[3]);
            if (word == 0) {
                if (out->wp + 1 > out->ep)
                    return fz_ioneedout;
                *out->wp++ = 'z';
