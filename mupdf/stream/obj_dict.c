...
fz_error* fz_deepcopydict(fz_obj** op, fz_obj* obj)
{
    fz_error* error;
    fz_obj* new;
    fz_obj* val;
...
static fz_error* growdict(fz_obj* obj)
{
    fz_keyval* newitems;
    int        newcap;
    int        i;
...
fz_obj* fz_dictgetkey(fz_obj* obj, int i)
{
    if (!fz_isdict(obj))
        return nil;
...
fz_obj* fz_dictgetval(fz_obj* obj, int i)
{
    if (!fz_isdict(obj))
        return nil;
...
fz_obj* fz_dictgets(fz_obj* obj, char* key)
{
    int i;
...
fz_obj* fz_dictget(fz_obj* obj, fz_obj* key)
{
    if (fz_isname(key))
...
fz_obj* fz_dictgetsa(fz_obj* obj, char* key, char* abbrev)
{
    fz_obj* v;
...
fz_error* fz_dictput(fz_obj* obj, fz_obj* key, fz_obj* val)
{
    fz_error* error;
    char*     s;
    int       i;
...
fz_error* fz_dictdel(fz_obj* obj, fz_obj* key)
{
    if (fz_isname(key))
