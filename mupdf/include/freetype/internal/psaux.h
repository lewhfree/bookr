...
#define T1_NEW_SIMPLE_FIELD(_ident, _type, _fname, _dict) {sizeof(_ident) - 1, _ident, T1CODE, _type, NULL, FT_FIELD_OFFSET(_fname), FT_FIELD_SIZE(_fname), 0, 0, _dict},

#define T1_NEW_CALLBACK_FIELD(_ident, _reader, _dict) {sizeof(_ident) - 1, _ident, T1CODE, T1_FIELD_TYPE_CALLBACK, (T1_Field_ParseFunc)_reader, 0, 0, 0, 0, _dict},

#define T1_NEW_TABLE_FIELD(_ident, _type, _fname, _max, _dict) {sizeof(_ident) - 1, _ident, T1CODE, _type, NULL, FT_FIELD_OFFSET(_fname), FT_FIELD_SIZE_DELTA(_fname), _max, FT_FIELD_OFFSET(num_##_fname), _dict},

#define T1_NEW_TABLE_FIELD2(_ident, _type, _fname, _max, _dict) {sizeof(_ident) - 1, _ident, T1CODE, _type, NULL, FT_FIELD_OFFSET(_fname), FT_FIELD_SIZE_DELTA(_fname), _max, 0, _dict},
...
typedef struct CFF_Decoder_FuncsRec_ {
    void (*init)(CFF_Decoder* decoder, TT_Face face, CFF_Size size, CFF_GlyphSlot slot, FT_Bool hinting, FT_Render_Mode hint_mode, CFF_Decoder_Get_Glyph_Callback get_callback,
...
#define IS_PS_TOKEN(cur, limit, token) \
    ((char)(cur)[0] == (token)[0] && ((cur) + sizeof((token)) == (limit) || ((cur) + sizeof((token)) < (limit) && IS_PS_DELIM((cur)[sizeof((token)) - 1]))) && ft_strncmp((char*)(cur), (token), sizeof((token)) - 1) == 0)
