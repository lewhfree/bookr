...
fz_error* fz_newtextnode(fz_textnode** textp, fz_font* font)
{
    fz_textnode* text;
...
void fz_droptextnode(fz_textnode* text)
{
    fz_dropfont(text->font);
    fz_free(text->els);
}

fz_rect fz_boundtextnode(fz_textnode* text, fz_matrix ctm)
{
    fz_matrix trm;
    fz_rect   bbox;
    fz_rect   fbox;
...
static fz_error* growtext(fz_textnode* text, int n)
{
    int        newcap;
    fz_textel* newels;
...
fz_error* fz_addtext(fz_textnode* text, int cid, float x, float y)
{
